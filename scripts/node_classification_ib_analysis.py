...
sys.path.append('../../')
sys.path.append('../../../')
from DGIB.model_osr import DGIBNN,DGIBNN_mlt,DGIBConv,compare_classwise_homophily_all,compare_classwise_neighbor_label_dist,compare_class_similarity,compute_distribution_shift
...
def train(model: DGIBNN_mlt, data, optimizer, device, args, epoch):
    ...
    # OOD 概率
    p_ood_score = torch.sigmoid(alpha_energy * (energy_test_all - threshold))
...
def main():
    ...
    parser.add_argument('--attack', type=str, default='dice', help='dice, MetaSelf')
    ...
    shift_clean = compute_distribution_shift(data.edge_index, data.y, data.y.max().item() + 1, data.train_mask, data.test_mask)
    shift_attacked = compute_distribution_shift(adj, data.y, data.y.max().item() + 1, data.train_mask, data.test_mask)
    print(f"Clean Graph KL shift: {shift_clean:.4f}")
    print(f"Attacked Graph KL shift: {shift_attacked:.4f}")
    ...
    plt.plot(ixz, label="IXZ Loss")
    plt.plot(ia, label="IA Loss")
    plt.plot(i_both, label="IA + IXZ Loss")
    plt.plot(i_yz_list, label="I(Y;Z) Loss")
    plt.plot(i_yoodz_list, label="I(Y;Z) OOD Loss")
