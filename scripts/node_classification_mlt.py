...
sys.path.append('../../')
sys.path.append('../../../')
from DGIB.model_mlt import DGIBNN,DGIBNN_mlt
...
def train(model, data, optimizer, device, args, epoch):
    model.train()
    optimizer.zero_grad()
    x_all = [data.x.to(device)]
    edge_index_all = [data.edge_index.to(device)]
    # Forward through DGIBNN
    embeddings_list, embeddings_energy, ixz_loss_s, struct_kl_loss_s, ixz_loss_1, struct_kl_loss_1, ixz_loss_2, struct_kl_loss_2 = model(x_all, edge_index_all)
    ...
    z_pos_hid = embeddings_energy[0]  # 假设第一个分支输出是 ID 嵌入
    z_pos = F.elu(z_pos_hid)
    z_pos = F.dropout(F.normalize(z_pos, dim=-1), p=0.5, training=True)
    outs = F.log_softmax(model.branch2_cls(z_pos), dim=1)
    loss_cls_openset = F.nll_loss(outs[data.train_mask], y_true[data.train_mask])
    
    energy_ind = model.energy_net(z_pos_hid[data.train_mask]).squeeze()
    # print('energy_ind', energy_ind, len(energy_ind))
    # 采样伪嵌入并计算能量
    ...
    # loss_openset = (energy_ind - energy_sample + energy_ind.pow(2) + energy_sample.pow(2)).mean() 
    # print(loss_openset, loss_cls_openset)
    # Combine with DGIB losses
    loss = loss_cls \
           + args.lambda_ixz * ixz_loss_s \
           + args.lambda_struct * struct_kl_loss_s \
           + args.lambda_ixz * ixz_loss_1 \
           + args.lambda_struct * struct_kl_loss_1
...
def test(model, data, device):
    ...
    x_all = [data.x.to(device)]
    edge_index_all = [data.edge_index.to(device)]
    embeddings_list, embeddings_energy,  _, _, _, _, _, _ = model(x_all, edge_index_all)
...
def main():
    ...
    parser.add_argument('--patience', type=int, default=600, help='Early stopping patience')
