...
def test(model: DGIBNN_mlt, data, device, epoch):
    ...
    auroc = roc_auc_score(test_true_labels, test_scores.cpu())  # 加负号，让OOD分数更高
...
def main():
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
       ...
       # 在main函数的训练循环中修改打印语句
        if epoch == 1 or epoch % 10 == 0:
            ...
            print(f"Openset Detection - AUROC: {accs['openset'][0]:.4f}, AUPR: {accs['openset'][1]:.4f}, FPR95: {accs['openset'][2]:.4f}")
            print(f"Multi-class (Known + OOD) - Acc: {accs['multi_class']['acc']:.4f}")
            print(f"F1 Score - Macro: {accs['multi_class']['f1_macro']:.4f}, Weighted: {accs['multi_class']['f1_weighted']:.4f}")   # model.visualize_energy_distributions(neg_energy_ind, neg_energy_openset, title="Energy Distribution")
    ...
    print(f"\nBest validation/test overall accuracy/test ind accuracy/openset detection: {best_val:.4f}/{best_overall_acc_test:.4f}/{best_overall_f1_test:.4f}/{best_openset_metrics}, model saved to {args.save_path}")
    ...
    plt.figure()
    plt.plot(test_acc_list, label="ID Accuracy (Known in Test)")
