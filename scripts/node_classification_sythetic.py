...
sys.path.append('../../')
sys.path.append('../../../')
from DGIB.model_sythetic import DGIBNN,DGIBNN_mlt,visualize_embeddings_tsne, visualize_energy_distributions
...
def train(model, data, optimizer, device, args, epoch, warm_up_epoch=60):
    ...
    embed_last_hid, logit, ixz_loss, struct_kl_loss = model(x_all, edge_index_all)
    embed_last_hid, logit = embed_last_hid[0], logit[0]
    outs = F.log_softmax(logit, dim=1)
    ...
    #     ###
    #     probs = model.d_phi(embed_last_hid).detach().cpu()
    ...
    if epoch > 20:
        # --------------------------- 伪 OOD 对齐（加权版本） ---------------------------
        model.d_phi.eval()
        z_pos_hid = embed_last_hid  # 假设第一个分支输出是 ID 嵌入
        z_pos_hid = F.elu(z_pos_hid)
        z_pos_hid = F.dropout(F.normalize(z_pos_hid, dim=-1), p=0.5, training=True)
        energy_ind = model.energy_net(z_pos_hid[data.train_mask]).squeeze()

        energy_test_ood = model.energy_net(z_pos_hid[data.unknown_in_unseen_mask]).squeeze()
        ...
        if epoch % 50 == 0:
            # visualize_embeddings_tsne(embed_last_hid, z_sample, data)
            visualize_energy_distributions(energy_ind, energy_test_ood)
        ...
        # energy_all = torch.cat([energy_ind, energy_sample], dim=0)  # [2 * N_id]
        # target = torch.cat([
        #     torch.ones_like(energy_ind),
        #     torch.zeros_like(energy_sample)
        # ], dim=0)
        # logits = -energy_all
        # loss_uncertainty = F.binary_cross_entropy_with_logits(logits, target)

        loss_uncertainty = torch.mean(F.relu(energy_ind - 0) ** 2 + F.relu(0 - energy_sample) ** 2)
        # loss_uncertainty = (energy_ind.mean() - energy_sample.mean())
...
def test(model, data, device):
    ...
    x_all = [data.x.to(device)]
    edge_index_all = [data.edge_index.to(device)]
    embed_last_hid, logit,  _, _,  = model(x_all, edge_index_all)
    embed_last_hid, logit = embed_last_hid[0], logit[0]
    outs = F.log_softmax(logit, dim=1)
    # print('outs', outs)
    y_true = data.y.to(device)
    ...
    embed_last_hid = F.elu(embed_last_hid)
    embed_last_hid = F.normalize(embed_last_hid, dim=-1)
    energy = - model.energy_net(embed_last_hid).squeeze()
    
    test_ind_score, test_openset_score = model.detect(energy.to(device), data.to(device))
    auroc, aupr, fpr, _ = get_measures(test_ind_score.cpu(), test_openset_score.cpu())
    accs['openset'] = [auroc] + [aupr] + [fpr]
    
    # test_ind_score, test_openset_score = model.detect_logit(logit.to(device), data.to(device))
    # auroc, aupr, fpr, _ = get_measures(test_ind_score.cpu(), test_openset_score.cpu())
    # accs['openset'] = [auroc] + [aupr] + [fpr]
    # # print('2', accs['openset'] )
